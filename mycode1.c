...
{
	/* your code here */
	memcpy(process+(p-1)*sizeof(CONTEXT),c,sizeof(CONTEXT));
}
...
{
	int magic = 0;
	SaveContext(process+(GetCurProc()-1)*sizeof(CONTEXT));
	if (magic==1){
		return cur_pid;
	}
	else{
	...
	cur_pid=GetCurProc();
	RestoreContext(process+(p-1)*sizeof(CONTEXT));
}
